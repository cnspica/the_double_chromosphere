/*
 * This program is just for fun, No efficiency
 *
 *      ^_^ Good Luck ^_^
 *
 *author:3null
...
/*
 *Get one ticket, include six red balls and one blue ball
 */
void random_a_ticket(ticket * ticket)
{
	...
	offset +=
		random_a_set_of_number(BLUE_NUMBER,
							   (char *)(ticket->ticket_number + offset));
...
char find_uint_in_array(unsigned long long finder, unsigned long long *array,
...
void choice_tickets(unsigned long long product_number,
					unsigned long long choice_number)
{
...
int main(int argc, char *argv[])
{
	...
	global_ticket = (ticket *) malloc(product_number * sizeof(ticket));

	if (NULL == global_ticket) {
		printf("Alloc memory failed, maybe tickets = lld% is so big\n",
			   product_number);
		return -1;
	}

	choiced_seq_addr =
		(unsigned long long *)malloc(choice_number *
